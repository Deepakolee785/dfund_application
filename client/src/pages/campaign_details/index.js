...
const CampaignDetails = () => {
		...
		},
		{
			onSuccess: data => {
				...
				const {
					...
					events: {
						LogContributionSent: {
							returnValues: { amount, contributor, projectAddress, isApprover },
						},
					},
				} = data
	...
	useEffect(() => {
		setIsCreator(accounts.length > 0 && data && accounts[0] === data.creator)
	}, [data, accounts])
	// const getCaptcha = () => {
					...
					<Col>
						<div style={{ marginLeft: '2rem' }}>
							...
							>
								...
								>
									{balance !== 0 && `${fromWeiToEther(web3, balance)}`}
								</span>
								ETH
							</p>
							<p
							...
							>
								...
								of {data ? fromWeiToEther(web3, data.goalAmount) : ''} ETH
								funded
							</p>
							<Progress
								strokeColor='#5F66F1'
							...
							>
								<span style={{ fontSize: '1rem', color: 'red' }}>*</span> You
								can und more than{' '}
								{data ? fromWeiToEther(web3, data.minimumContrubution) : ''} ETH
								to be approver of this campaign
				...
				<Tabs defaultActiveKey='2'>
					...
					>
						...
						{isCreator && isAuthenticated && data && data.active && (
							<Button type='primary' variant='danger' onClick={kill}>
								Terminate campaign
							</Button>
						)}
					</TabPane>
